...
import {
  ...
  totalEnemiesAlive,
  totalRaidersAlive,
} from '../../../../../../models/TowerFloorBattlefield';
import {
  addTowerFloorBattlefieldEnemies,
...
export async function startRoundCommand(userRequesting: User) {
  return withTowerTransaction(async (transaction) => {
    ...
    if (totalEnemies > ZERO) {
      ...
      await Promise.all(enemies.map((enemy) => enemy.reloadFullEnemy(transaction)));
      await theTowerNotifyInPrivate(
      ...
      if (towerFloor.number === towerFloor._towerGame?.height) {
        ...
        const lootPrize = await towerGameEngine.generateLoot(towerFloor, raider, transaction);
        await raider.reloadFullInventory(transaction);
        const healthKitsLooted = lootPrize.filter(
          (prize) => prize.name === TOWER_HEALTHKITS.COMMON
        ).length;
        const healthKit = lootPrize.find((prize) => prize.name === TOWER_HEALTHKITS.COMMON);
        const healthKitsNeededToFullHP = Math.ceil(
          (MAX_RAIDER_HEALTH - previousHealth) / (healthKit?._healthkit?.healingPower ?? ONE)
        );
